...
void QCamera::look(float yaw, float pitch, float roll)
{
	m_yaw += yaw;
	while (m_yaw >= 360)
		m_yaw -= 360;
...
void QCamera::move(float x, float y, float z)
{
...
void QCamera::resetLook()
{
	m_front = { 0,0,-1 };
}

void QCamera::resetRotate()
{
	m_up = { 0,1,0 };
}

void QCamera::resetMove()
{
	m_pos = { 0,0,0 };
}

void QCamera::resetAll()
{
	resetLook();
	resetRotate();
	resetMove();
...
bool QCamera::eventFilter(QObject *obj, QEvent *ev)
{
