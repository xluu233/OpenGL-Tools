...
AssModel::AssModel(QString path, QOpenGLContext *context, QOpenGLShaderProgram *shaderProgram)
        : context(context)
...
AssModel *AssModel::createModel(QString path, QOpenGLContext *context, QOpenGLShaderProgram *shaderProgram) {
    return new AssModel(path,context,shaderProgram);
}
...
AssMesh *AssModel::processMesh(aiMesh *mesh, const aiScene *scene, aiMatrix4x4 model) {
    //  初始化网格
    AssMesh* m_mesh = new AssMesh(context->functions(),shaderProgram,model);
    // 遍历网格的每个顶点
